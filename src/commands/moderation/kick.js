...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    const reason =
      interaction.options.get('begrüngung')?.value || 'Kein Grund angegeben';
    ...
    try {
      ...
      await interaction.editReply(
        `User ${targetUser} wurde gekicked\nBegrüngung: ${reason}`
      );
    } catch (error) {
      console.log(`Beim Kicken ist ein Fehler aufgetreten: ${error}`);
    }
  },

  name: 'kick',
  description: 'Kicke einen User von dem Server',
  options: [
    {
      name: 'user',
      description: 'Denn User den du kicken möchtest',
